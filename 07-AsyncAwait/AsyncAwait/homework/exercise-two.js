...
function problemA () {
  ...
  let p1 = asyncReadFile('poem-two/stanza-01.txt').then(stanza => blue(stanza));
  let p2 = asyncReadFile('poem-two/stanza-02.txt').then(stanza => blue(stanza));
...
function problemB () {
  ...
  let proms = filenames.map(filename => {
    return asyncReadFile(filename).then(stanza => blue(stanza));
  });
...
function problemC () {
  ...
  for (let i = 1, p=asyncReadFile(filenames[0]); i <= filenames.length; i++) {
    p = p.then((stanza) => {
      blue(stanza);
      if (i === filenames.length) {
        ...
        return asyncReadFile(filenames[i]);
      }
    });
  };
}
